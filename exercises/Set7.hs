...
data State = Start | Error | Finished | WithEggs | WithEggsFlour | WithEggsSugar | WithEggsFlourSugar | Mixed
  deriving (Eq, Show)
...
step WithEggsFlour AddSugar = WithEggsFlourSugar
step WithEggsFlour _ = Error
step WithEggsSugar AddFlour = WithEggsFlourSugar
step WithEggsSugar _ = Error
step WithEggsFlourSugar Mix = Mixed
step WithEggsFlourSugar _ = Error
...
instance (Ord a) => Semigroup (Set a) where
  Set as <> Set bs = Set (sort $ nub (as ++ bs))
...
passwordAllowed pw (ContainsSome s) = any (`elem` s) pw
passwordAllowed pw (DoesNotContain s) = not (any (`elem` s) pw)
passwordAllowed pw (And a b) = passwordAllowed pw a && passwordAllowed pw b
passwordAllowed pw (Or a b) = passwordAllowed pw a || passwordAllowed pw b
