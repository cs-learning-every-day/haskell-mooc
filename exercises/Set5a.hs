...
toNat :: Int -> Maybe Nat
toNat 0 = Just Zero
toNat x = if x < 0 then Nothing else Just (PlusOne $ fromJust (toNat (x - 1)))
...
fromBin b = go 1 b
  where
    go power End = 0
    go power (O rest) = go (power * 2) rest
    go power (I rest) = power + go (power * 2) rest
