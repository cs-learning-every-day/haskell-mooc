...
longest :: (Ord a) => [[a]] -> [a]
longest xxs = minimumBy (comparing (\xs -> (-length xs, head xs))) xxs
...
incrementKey k pairs = map (\(key, val) -> (key, if key == k then val + 1 else val)) pairs
...
average :: (Fractional a, Real a) => [a] -> a
...
freqs = foldr (\x m -> Map.alter (\v -> Just $ maybe 1 (+ 1) v) x m) Map.empty
...
transfer from to amount bank
  ...
  | otherwise =
      if fromBalance < amount
        then bank
        else Map.insert from (fromBalance - amount) . Map.insert to (Map.findWithDefault 0 to bank + amount) $ bank
  where
